...
import { Loader2, User, Phone, ShoppingCart, XCircle, PlusCircle, Trash2, FileText } from 'lucide-react';
import { updateOrder, type State } from '@/app/actions';
...
export function EditOrderForm({ order, onUpdateSuccess }: { order: Order, onUpdateSuccess: (updatedOrder: Order) => void }) {
  ...
  const [name, setName] = useState(order.request.name);
  const [phone, setPhone] = useState(order.request.phone);
  const [notes, setNotes] = useState(order.request.notes ?? '');
  const [orderItems, setOrderItems] = useState<OrderItem[]>(order.request.items || []);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (showItemForm) {
      toast({
        ...
        description: "Aggiungi o annulla l'articolo prima di salvare.",
        variant: "destructive"
      });
      return;
    }
    ...
    startTransition(async () => {
      const result = await updateOrder(order.id, { success: false, message: null, errors: {} }, formData);
      if (result.success) {
        ...
        onUpdateSuccess(updatedOrder as Order);
      } else {
        toast({
          title: 'Errore',
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-h-[80vh] overflow-y-auto p-1 pr-4">
      <div className="space-y-4">
        <div className="relative">
          ...
          <Input id="name" name="name" value={name} onChange={e => setName(e.target.value)} placeholder="Nome e Cognome" className="pl-10 h-12" required />
        </div>
        <div className="relative">
          <Phone className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
          <Input id="phone" name="phone" value={phone} onChange={e => setPhone(e.target.value)} placeholder="Numero di Telefono" className="pl-10 h-12" required />
        </div>
      </div>
      ...
      <Card>
          ...
          <CardContent className="space-y-4">
                ...
                <div className="p-4 border rounded-lg space-y-4 bg-muted/50">
                    {currentImage && (
                        <div className="flex justify-center my-4">
                            <Image src={currentImage.imageUrl} alt={currentImage.description} width={150} height={150} className="h-40 w-auto object-contain transition-all duration-300" data-ai-hint={currentImage.imageHint} />
                        </div>
                    )}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                       ...
                       <div>
                          ...
                          <Input id="quantity" type="number" min="1" value={currentItem.quantity} onChange={(e) => setCurrentItem(prev => ({...prev, quantity: parseInt(e.target.value, 10) || 1}))} className="h-12" />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
            ...
            <div className="space-y-2">
                ...
                {orderItems.length === 0 && !showItemForm && (<div className="text-center py-8 border-2 border-dashed rounded-lg"><p className="text-muted-foreground">Il carrello è vuoto.</p></div>)}
            </div>
          </CardContent>
      </Card>

      <div>
        <Label htmlFor="notes" className="flex items-center gap-2 font-semibold mb-2">
          <FileText className="w-5 h-5" />
          Note
        </Label>
        <Textarea id="notes" name="notes" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Aggiungi note sull'ordine..." />
      </div>
      ...
      <div className="space-y-4 rounded-lg bg-muted/50 p-4">
       ...
       <Button type="submit" disabled={isPending} className="w-full text-lg py-6">
        {isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Salvataggio...</> : 'Salva Modifiche'}
      </Button>
    </form>
  );
}